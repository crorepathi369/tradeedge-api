...
# Symbols to skip during fetch (e.g. known Yahoo Finance failures)
...
def _files_modified_today() -> list[Path]:
    ...
    for f in DATA_DIR.glob("*.csv"):
        if f.stem.upper() in EXCLUDE_SYMBOLS:
            continue
...
def _all_files() -> list[Path]:
    ...
    files = sorted(
        (f for f in DATA_DIR.glob("*.csv") if f.stem.upper() not in EXCLUDE_SYMBOLS),
        key=lambda f: f.name
    )
    sm = DATA_DIR / "SECTOR_MAP.json"
    if sm.exists():
...
def _symbol_files(symbols: list[str]) -> list[Path]:
    ...
    for sym in symbols:
        if sym.upper() in EXCLUDE_SYMBOLS:
            print(f"  ⚠  {sym} is in EXCLUDE_SYMBOLS — skipping")
            continue
